pub mod encrypt {
    ...
    pub use mix_coloumns::mix_coloumns;
    pub use round_key::add_round_key;
    pub use shift_rows::{rot_word, shift_rows};
    pub use sub_bytes::{sub_bytes, sub_word};
    // pub use cipher::cipher;
    // pub use sub_bytes::sub_bytes;
}
...
pub mod utils {
    ...
    pub use helpers::GF256;
    pub use key_expansion::{expand_key, transpose};
}

pub mod aes128;

/// AES-128 Key-Block-Round Function:
/// Key Length (NK) : 4
/// Blocks (NB) : 4
/// Rounds (NR) : 10
/// Expanded Key / Key Schedule (NR + 1) * NB : 44
pub const NB: usize = 4;
pub const NK: usize = 4;
pub const NR: usize = 10;
pub type State = [[u8; NB]; 4];
pub type Key = [[u8; NK]; 4];
pub type KeySchedule = [[u8; NB]; (NR + 1) * NB];

pub trait KeyTrait {
    fn new() -> Self;
}
pub trait KeyScheduleTrait {
    fn new() -> Self;
}

impl KeyTrait for Key {
    /// Initialise `Key` to `0`'s.
    fn new() -> Self {
        [[0u8; NK]; 4]
    }
}

impl KeyScheduleTrait for KeySchedule {
    /// Initialise `KeySchedule` to `0`'s.
    fn new() -> Self {
        [[0u8; NB]; (NR + 1) * NB]
    }
}

/// The S-box used in the SubBytes() transformation is presented in hexadecimal form below.
/// For example, if s(1,1) = {53}, then the substitution value would be determined by the intersection
/// of the row with index ‘5’ and the column with index ‘3’. This would result in s(1,1) having
/// a value of {ed}.
